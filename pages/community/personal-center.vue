<template>
	<view class="personal-wrapper">
		<view class="profile-header">
			<view class="cover-wrapper" @click="changeCoverImage">
		...
		<view class="personal-list">
			<view v-if="personalTab === 'posts'">
				<view v-if="myPostList.length === 0" class="empty-state">你还没有发布过动态哦~</view>
				<view class="post-card" v-for="(post, index) in myPostList" :key="index">
					...
					<view class="post-footer">
						<view class="action-btn">
							<text class="color-normal">点赞 {{ post.likesCount || 0 }}</text>
						</view>
						<view class="action-btn">
							<text class="color-normal">评论 {{ post.commentsCount || 0 }}</text>
						</view>
						<view class="action-btn" @click="deleteMyPost(post.postId)">
...
export default {
	...
	methods: {
		...
		// 供父组件调用的触底加载
		loadMore() {
			if (this.isLoggedIn && this.personalTab === 'posts') {
				this.myPostPage++;
						...
						success: (res) => {
							...
							if (res.data.code === 200) {
								...
								this.userInfo = {
									...
									avatar: data.avatarUrl || '/static/default-avatar.png', // 修改了这里：对应后端的 avatarUrl
									signature: data.bio || '这个人很懒，什么都没留下~',         // 修改了这里：对应后端的 bio
		...
		changeCoverImage() {
			...
			uni.chooseImage({
				...
				success: (res) => {
					...
					this.uploadCoverToUniCloud(tempFilePath);
				}
			});
		},
		// 上传到 uniCloud
		...
		goToNotices() {
			if (!this.isLoggedIn) return uni.navigateTo({
				url: '/pages/login/login'
			});
			uni.showToast({
				title: '消息列表开发中...',
